...
function mapCustomer(r: Record<string, unknown>): Customer { return { id: r.id as string, name: r.name as string, age: (r.age as string) ?? '', mobile: r.mobile as string, altNumber: (r.alt_number as string) ?? '', location: (r.location as string) ?? '', isStudent: (r.is_student as boolean) ?? false, visitCount: (r.visit_count as number) ?? 0, createdAt: (r.created_at as string) ?? new Date().toISOString(), googleContactResourceName: (r.google_contact_resource_name as string) ?? undefined }; }
